...
\caption{Regime map: $\Delta$F1 (CV2 $-$ CoT-SC) vs.\ median context length across 6 benchmarks (HotpotQA + 5 LongBench subsets). Filled markers with dark outline = paired-significant. CV2 wins concentrate on multi-passage contexts at 5--12k tokens (green squares); short single-doc / short multi-hop (qasper, 2wikimqa) are near-zero; single-file dense evidence and ultra-long narrative are documented boundaries.}
\label{fig:regime_map}
\end{figure}

\noindent\textbf{Summary.} Figure~\ref{fig:regime_map} synthesizes the 6-benchmark regime map: GERS-CV2 wins where reasoning requires bridging across \emph{multiple passages with heavy distractors} at \emph{5--12k tokens}. It does not help on short single-file QA (HotpotQA bridge), short multi-hop with concentrated evidence (2wikimqa), single-doc scientific QA (qasper), deep composite chains exceeding reasoner error-propagation tolerance (2Wiki bridge\_comp), or contexts beyond model capacity (narrativeqa). This regime coincides with retrieval-augmented multi-document QA.
...
\noindent\textbf{Regime boundary sharpening: two more LongBench subsets (n=200 each).} We extended the LongBench evaluation to two additional English QA subsets that fall \emph{outside} the target regime, to test the regime hypothesis. On \emph{qasper} (single-document scientific QA, median $\sim$3.4k tok), CV2 shows a small non-significant positive trend: F1 0.290 vs.\ 0.272, dF1 $+0.018$, 95\% CI $[-0.016, +0.054]$, McNemar $p{=}1.000$. On \emph{2wikimqa} (short multi-hop, median $\sim$4.2k tok), CV2 shows a small non-significant negative trend: F1 0.681 vs.\ 0.700, dF1 $-0.021$, 95\% CI $[-0.076, +0.035]$, McNemar $p{=}0.324$. Neither reaches significance in either direction. This is exactly the expected pattern for regimes outside the 5--12k multi-passage sweet spot---short single-document QA (qasper) and short multi-hop with concentrated evidence (2wikimqa) do not require sub-question isolation, so decomposition neither helps nor hurts materially. Combined with \S4.2 (significant CV2 wins on multifieldqa\_en and musique, joint failure on narrativeqa), the 5-subset LongBench picture forms a coherent regime map: CV2 wins where evidence is scattered across multiple passages at 5--12k tokens, and is neutral or slightly worse elsewhere.
