...
**显著性检验**。由于测试集规模有限（n=100），本文对所有主结果报告 bootstrap 95% 置信区间（10000 次重采样）与配对 McNemar 检验（精确二项检验），以避免将小样本下的随机波动误读为方法增益。
...
**HotpotQA（GERS 主战场）**。GERS-SC 取得 EM=0.270、F1=0.395，相比 CoT-SC 的 EM=0.200、F1=0.284，EM 提升 7pt、F1 提升 11pt。**该差距在统计上显著**：表 2 报告了 bootstrap 95% 置信区间与 McNemar 配对检验结果——GERS-SC 与 CoT-SC 的 EM 差值为 +0.070，95% CI [+0.020, +0.130]（不跨零），McNemar 检验 p=0.039（GERS-SC 答对而 CoT-SC 答错 8 例，反向 1 例）；与 Standard CoT 相比 p=0.021。因此在 n=100 规模下，GERS-SC 的优势并非随机波动。值得注意的是，CoT-SC+GERS 重排（仅在选择阶段用 GERS 校验分，不进行图结构执行）与 CoT-SC 同分（0.284），表明 GERS-SC 的增益主要来自图结构分解执行 + 结构质量选择的协同，而非单纯的选择阶段重排（详见 4.6 节消融）。

**表 2** 显著性检验（HotpotQA, n=100）

| 对比 | EM 差值 | 95% bootstrap CI | McNemar (b/d) | p 值 |
|------|:-:|:-:|:-:|:-:|
| GERS-SC vs CoT-SC | +0.070 | [+0.020, +0.130] | 8 / 1 | **0.039** |
| GERS-SC vs Standard CoT | +0.100 | [+0.030, +0.180] | 13 / 3 | **0.021** |
| CoT-SC vs Standard CoT | +0.030 | [−0.020, +0.080] | 5 / 2 | 0.453 |

> b = GERS-SC 答对而对照答错的样本数，d = 反向。CI 不跨零且 p<0.05 视为显著。
...
GERS-SC 的有效性依赖于 Consistency Score 能区分推理路径质量。表 3 展示了 Consistency Score 在两个数据集上的取值。

**表 3** Consistency Score 分析（GERS-SC, n=100）
...
GERS-SC 生成 K=3 条完整推理 DAG，每条 DAG 包含分解、逐步执行、汇总与一致性校验，LLM 调用量高于线性基线。表 5 对比了各方法的平均 LLM 调用次数与单题延迟。

**表 5** 计算成本对比（HotpotQA, n=100, 4 线程并行, 单题平均）

| 方法 | LLM 调用次数/题 | 单题延迟 | EM |
|------|:-:|:-:|:-:|
| Zero-Shot | 1 | 1.8s | 0.160 |
| Standard CoT | 1 | 6.2s | 0.170 |
| CoT-SC (N=3) | 3 | 20.0s | 0.200 |
| CoT-SC+GERS 重排 | 3 + U (≤2) | 22.9s | 0.190 |
| GERS+自适应 | 1~5（自适应） | 5.3s | 0.260 |
| GERS-SC (K=3) | 3×(分解+执行+校验) ≈ 12~18 | 17.3s | **0.270** |

> U 为去重后 unique 答案数；GERS+自适应因复杂度路由，简单题仅 1 次调用、复杂题约 5 次；GERS-SC 的 K 条 DAG 中每条约 4~6 次调用（分解 1 + 子问题执行 N + 汇总 1 + NLI 校验）。

**成本-收益权衡**。GERS-SC 的 LLM 调用量约为 Standard CoT 的 12~18 倍、CoT-SC 的 4~6 倍，但带来 EM +7pt（vs CoT-SC）、F1 +11pt 的显著提升。相比之下，ToT[8] 等搜索方法每题需 6 次以上调用且在本任务上易因盲目搜索失效。GERS+自适应则以接近 Standard CoT 的成本（5.3s）取得 EM=0.260，是成本敏感场景下的折中方案。本文认为，GERS-SC 的额外计算开销换取了可量化、可解释的推理质量提升（Consistency Score），在离线高质量推理场景下是合理的权衡；而对延迟敏感的在线场景，可采用 GERS+自适应。

### 4.6 消融实验

为分离 GERS-SC 各组件的贡献，本文设计消融对照（表 6）。其中"图执行"指子问题 DAG 分解 + 拓扑执行，"质量选择"指用 Consistency Score 在多条 DAG 间选优。

**表 6** 消融实验（HotpotQA, n=100）

| 配置 | EM | F1 | 说明 |
|------|:-:|:-:|------|
| **GERS-SC（完整）** | **0.270** | **0.395** | 图执行 + K=3 质量选择 + 答案类型回扣 |
| w/o 图执行（= CoT-SC+GERS 重排） | 0.190 | 0.284 | 仅答案选择阶段用 GERS 分，无 DAG 执行 |
| w/o 质量选择（= CoT-SC 等权投票） | 0.200 | 0.284 | 多采样但不按 CS 选优，等权投票 |
| GERS+自适应（K=1，无多路选优） | 0.260 | 0.382 | 单条 DAG，无质量选择 |
| w/o 答案类型回扣（2Wiki bridge_comp） | — | 0.238→0.476 | 答案类型回扣使复合题 F1 翻倍（见 4.4） |

**消融结论**：
- **图执行是核心**：移除图执行（退化为 CoT-SC+GERS 重排）后 EM 从 0.270 降至 0.190，低于 CoT-SC（0.200），证明增益主要来自 DAG 分解与拓扑执行，而非单纯的选择阶段重排。
- **质量选择有正向贡献**：GERS-SC（K=3, CS 选优, 0.270）优于 GERS+自适应（K=1, 无选优, 0.260），Consistency Score 选优在中等跳数上贡献 +1pt EM、+1.3pt F1。
- **答案类型回扣显著影响复合题**：在 2Wiki bridge_comparison 上，该机制使 F1 从 0.238 提升至 0.476（翻倍），是答案对齐的关键。
