...
\textbf{Figure 1.} GERS reasoning flow.\\[2pt]
Input $Q,C$ $\to$ ①decompose $\to$ ②build DAG $\to$ ③topological sort $\to$ ④execute $\to$ ⑤aggregate (answer-type alignment) $\to$ ⑥consistency check (bidirectional cross-validation) $\to$ [GERS-SC: select best of $K$ DAGs] $\to$ output $A,G,S$
}}
\end{figure}
