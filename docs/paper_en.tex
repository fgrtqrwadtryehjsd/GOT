...
\item \textbf{Empirical contribution}: The best configuration GERS-CV2 achieves EM=0.302, F1=0.413 on HotpotQA (500 samples), a paired-significant improvement over CoT-SC (McNemar $p=0.029$; F1 +4pt), though with a marginal bootstrap effect-size interval that we report transparently.
...
\caption{Main results on HotpotQA (n=500). GERS-CV2 achieves the best EM/F1; the CoT-SC+GERS rerank variant stays on par with CoT-SC, indicating graph scores must participate in generation and verification rather than post-hoc reranking.}
\end{figure}
...
\caption{2WikiMultiHopQA per-type F1 (n=100, diagnostic). GERS-CV2 matches CoT on comparison but lags on bridge\_comparison due to upstream entity error propagation.}
\end{figure}
...
GERS-CV2 (single-path version) achieves the best EM=0.302 with $\sim$6--8 LLM calls and 6.6s latency---fewer calls than CoT-SC's 3$\times$ sampling at comparable wall-clock, while leading by 4pt F1---thanks to adaptive routing (simple questions skip decomposition) plus backward verification (only on complex questions). The multi-path GERS-SC ($\sim$12--15 calls) costs more with no performance gain, reinforcing the limited value of multi-path selection here.
...
\textbf{Confirmation bias of reverse verification.} Anchoring backward re-derivation on the final answer $A$ can bias the model toward explanations consistent with $A$, risking spurious agreement. We mitigate this by instructing the model to derive from the context rather than copy $A$. We did not run a full ablation of anchor vs.\ context-only backward verification or adversarial-answer perturbation; this remains a limitation, and future work will use adversarial anchors or external verifiers to further reduce the bias.
